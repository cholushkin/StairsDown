    ...
    public class SegmentArchitect
    {
            ...
            TopologyType topologyType)
        {
            ...
            if (parentNode != null)
            {
                ...
                bounds = CreateBoundsForChild(parentBounds, buildDirection, meta.AABBs[sizeIndex], offsetFromParent, 1f, rot);
            }
            else // parentNode is a proxy empty (establishment) node
            {
                bounds = CreateBounds(meta.AABBs[sizeIndex], offsetFromParent, 1f, rot);
            }
            ...
            {
                Geometry = new Blueprint.Segment.TopologySegment.ChunkGeometry
                {
                    Bounds = bounds, 
        ...
        public static Bounds CreateBoundsForChild(Bounds parentBounds, Vector3 side, Vector3 childSize, Vector3 offset, float childScale = 1f, int rot = 0)
        {
            if (rot == 270)
            {
                childSize = new Vector3(childSize.z, childSize.y, childSize.x);
            }
        ...
        public static Bounds CreateBounds(Vector3 size, Vector3 offset, float scale = 1f, int rot = 0)
        {
            if (rot == 270)
            {
